...
enc/auth keys - v2-era raw-password images remain READABLE (deprecated
v2 read path) but are never written anymore; re-embed them to upgrade.

## Migrating a fielded image

1. Extract with the matching tool: `python/extract.py <old.png> <payload.bin>`
   (reads v1, v2, and current images).
2. Re-embed current: `python/stego_cli.py hide <payload.bin> --cover <cover.png> -o <new.png>` (add `--password/--auth/--scatter` as needed).
